...
// ── Dónde está la frontera con el TEXTO ──────────────────────────────────────────────────
// Aquí NO. Este módulo trabaja sobre el árbol ya leído y no sabe cómo se escribió: no importa el
// parser, ni el preprocesado de la notación. La lectura de una ecuación escrita vive en
// `extraer.ts`, que es un envoltorio de veinte líneas sobre esto.
//
// La separación no es estética. Mientras las dos cosas estaban en el mismo archivo, cualquiera
// que quisiera polinomios exactos —el núcleo de `CAS/` entre otros— se llevaba en su grafo de
// dependencias el parser entero, `formatoExpr` y `expr/` completo, y el invariante de que el
// núcleo no depende de la lectura del texto era falso sin que ninguna prueba lo notara.

// `Nodo` se toma de donde está DEFINIDO (`expr/nodo.ts`, que solo depende de mathjs) y no de
// `formatoExpr.ts`, que lo reexporta. La diferencia no es de estilo: `formatoExpr` arrastra
// `expr/` entero, y con ello media docena de módulos de presentación entraban en el grafo de
// dependencias de quien solo quería un tipo.
...
export const entera = (p: Polinomio2): Fraccion => ({ num: p, den: constante2(UNO) });
 ...
 */
export function fraccionDe(n: Nodo): Fraccion | null {
...
/**
 * Los dos lados, ya leídos como fracciones de polinomios, llevados a «esto vale cero».
 *
 *   izq/denIzq = der/denDer  ⇔  izq·denDer − der·denIzq = 0
 *
 * `null` cuando la ecuación no dice nada (`y = y` da 0 = 0) o cuando el grado se dispara por
 * encima de lo que la resultante puede resolver en un tiempo razonable.
 */
export function polinomicaDeFracciones(
  izquierda: Fraccion, derecha: Fraccion
): EcuacionPolinomica | null {
