...
export interface ObjetoBase {
  ...
  /**
   * La restricción de dominio escrita en el bloque acota una variable que este objeto NO tiene
   * (`\sin x {0 ≤ t ≤ 3}`). El objeto se construye VACÍO —no se recorta por la variable que sea,
   * ni se ignora lo escrito—, y esta marca es lo que permite al host decir POR QUÉ no hay curva
   * en vez de dejar el genérico "no definida en ℝ".
   *
   * Es un aviso ESTRUCTURADO, sin texto: el modelo no sabe en qué idioma se lee (misma división
   * que los avisos de obs-trig).
   */
  readonly avisoRestriccion?: "variableAjena";
}
