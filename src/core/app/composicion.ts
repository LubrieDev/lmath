...
import { lectorExacto } from "../../math/ordenada";
 ...
 */
/**
 * El proveedor de geometría de una ecuación, y los OBJETOS de los que sale.
 *
 * Devuelve los dos porque hay un segundo interesado en el objeto matemático: el lector exacto de
 * la ordenada (`mate/ordenada`). Construirlo aparte obligaría a compilar la función dos veces por
 * ecuación, y —peor— abriría la puerta a que el crosshair leyera un objeto construido con otras
 * opciones que el que se dibuja.
 */
function proveedorDeEcuacion(
  ec: string, id: string
): { proveedor: ProveedorGeometria; objetos: ObjetoMatematico[] } {
  const ramas = tieneDobleSigno(normalizarEntrada(ec)) ? expandirDobleSigno(normalizarEntrada(ec)) : [ec];
  const objetos = ramas.map((e) => construirObjeto(e, id));
  const proveedor = objetos.length === 1
    ? crearProveedor(objetos[0])
    : new ProveedorUnion(id, objetos.map((o) => crearProveedor(o)));
  return { proveedor, objetos };
}
...
function objetoEscena(ec: string, id: string, indiceColor: number, ocultarPuntosEje = false): ObjetoEscena {
  const { proveedor: base, objetos } = proveedorDeEcuacion(ec, id);
  ...
  // Lector EXACTO de la ordenada, si esta curva admite uno. Se exige UNA sola rama: `y = ±√(…)`
  // se expande en dos curvas y ahí «la y en esta x» vuelve a ser ambigua, que es justo la
  // condición que `lectorExacto` pide para existir.
  const lectorY = objetos.length === 1 ? lectorExacto(objetos[0]) : null;
  return lectorY ? { proveedor, estilo, lectorY } : { proveedor, estilo };
}
