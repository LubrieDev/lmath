...
import type { LectorOrdenada } from "../../math/ordenada";

export interface ObjetoEscena {
  readonly proveedor: ProveedorGeometria;
  readonly estilo: Estilo;
  /**
   * Lector EXACTO de la ordenada, cuando la curva admite uno (`mate/ordenada.lectorExacto`).
   * Lo pone quien construye la escena, porque es quien tiene el objeto matemático; la escena
   * solo elige entre él y la polilínea.
   *
   * Su razón de ser: la polilínea es para VISUALIZAR, y deja de ser una fuente de verdad
   * aceptable en cuanto hay forma de evaluar la expresión. Sin esto, el crosshair leía la y
   * interpolando entre vértices y el valor cambiaba con el zoom. Sin fijar —implícitas,
   * paramétricas y polares— se sigue leyendo la rama, que ahí es la única fuente que hay.
   */
  readonly lectorY?: LectorOrdenada;
}
...
export class Escena {
  ...
  /**
   * y sobre la curva SELECCIONADA en un x de mundo. La usan el carril y el crosshair, y es el
   * ÚNICO sitio donde se decide de dónde sale ese número.
   *
   * Con lector exacto (la explícita `y = f(x)`) se EVALÚA la función: el valor es el mismo con
   * cualquier zoom y con cualquier paneo, porque no depende del trazado. Sin él —implícitas,
   * paramétricas, polares— se interpola la polilínea como siempre: ahí «la y en esta x» no tiene
   * respuesta única y la rama sabe por dónde pasó el trazador, que es la mejor fuente disponible.
   *
   * Que la elección viva aquí y no en el crosshair es a propósito: el renderer dibuja, no decide
