...
import { inversionTrig, despejeTrigCuadratico, type TrigInvertible } from "./despejeInverso";
...
// El despejador se reparte en tres piezas; esta conserva el ALGORITMO (llevar todo a
// D = lhs − rhs, elegir estrategia, validar) y delega el resto.
import {
  conDominio, contieneY, desParen, despejeAbsoluto, despejeMultiplicativo,
  despejePotencia, despejeRaiz, despejeRaizDePotencia, despejeReciproco,
  despejeTrigCero, despejeTrigInverso, exponenteEntero, exponenteY,
  limpiarRHS, linealEnY, renderProducto,
} from "./despeje/estrategias";
import { mcdEnteros } from "./despeje/aritmetica";
import { embellecerConstantes } from "./despeje/presentacion";
import { ramaReal, solucionValida, evaluadorDe } from "./despeje/verificacion";
import {
  contieneVariable, terminos, factores, flip, renderTerminos, renderCanonico,
  racionalizarFracciones, formatearCanonico, profundidadFraccion, type Termino, type Factor,
  type Nodo,
} from "./formatoExpr";
 ...
 */
function despejeCuadratico(D0: Nodo, DVal: Nodo = D0): ResultadoDespeje | null {
...
function despejeLinealEnY(D: Nodo): ResultadoDespeje | null {
 ...
 *  reproduce la curva. */
function despejeRadicales(D0: Nodo, DVal: Nodo): ResultadoDespeje | null {
  let D = D0;
...
function despejeSinDenominadores(D: Nodo): ResultadoDespeje | null {
...
function despejarAnidado(ecuacion: string): ResultadoDespeje | null {
...
function rhsCompleto(r: ResultadoDespeje | null): string | null {
...
function despejePorInversion(L: Nodo, R: Nodo): ResultadoDespeje | null {
...
function despejar(ecuacion: string): ResultadoDespeje | null {
  ...
  if (conY.length === 1) {
    ...
    const trigCero = despejeTrigCero(conY[0], derecha, despejar);
    ...
    const recip = despejeReciproco(conY[0], derecha, despejar);
...
export function despejarEcuaciones(ecuaciones: readonly string[]): string[] {
  return ecuaciones.map((entrada) =>
    transformarSinRestriccion(entrada, (ec) => embellecerConstantes(despejar(ec)?.ecuacion ?? ec))
  );
}
