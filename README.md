...
| ` ```obs-vector ` | **Vector notation**: one card per line, typeset as what it is — `v = (3,2)` gets the arrow of `\vec{v}`, `A = (1,2)` does not, `F(x,y) = (-y,x)` is a field. Vectors and points with numbers are also drawn on a plane, with an **ⓘ** panel for what follows from them (magnitude, direction, dot product, angle, distance). |
...
- An ⓘ panel for `obs-vector` that reports only what *follows* from what you wrote — magnitude, direction, unit vector, and for exactly two of them the dot product, the angle, the determinant and the areas — exact (`√13`) whenever the components are integers.
...
- Values are **exact when they were earned** — with integer components the magnitude of `(3,2)` is
  ...
  of [Settings](#settings), the same one `obs-trig` uses.
...
separates them behind the same button bar `obs-derivate` and `obs-integral` use — the main button
...
`obs-graph` and `obs-system`, one interval per equation; `obs-derivate` and `obs-integral` do not
take it yet.
...
alongside the formula: its button takes you there and back, the same way `obs-vector` switches
...
`obs-derivate` and `obs-integral` do not take parameters yet. Values are not written back to the
...
[obs-trig](#obs-trig) for its own controls. The plane of `obs-vector` has none of these either:
...
An empty block shows the message *No function* instead of an error. `obs-trig` is the exception: an empty block there is a complete figure at 30°.

`obs-vector` follows the same rule with its own words. Its plane is always there, and when there is
...
The plugin normalizes different formats before evaluating them with [mathjs](https://mathjs.org/). This applies to all six blocks, which share the same parser — `obs-trig` and `obs-vector` included: the first reads the angle you write with exactly this machinery, and the second reads each component of a pair with it.
...
angle of an `obs-trig` to the starting point.
...
- **Angle unit** — degrees, radians or gradians (degrees by default). Presentation only: it changes how angles are *written*, never how a block is read, so a bare number is still radians whatever you pick. Each `obs-trig` block also has a **θᴅ / θʀ / θɢ** chip that overrides it for that block until the note is re-rendered. It is also the unit of the angles in the ⓘ panel of `obs-vector`, which has no chip of its own.
...
- `obs-vector` writes, draws and reports; it does not operate. `w = u + v` is typeset and left as it is, and nothing in the block — the ⓘ panel included — produces a vector you did not write. There are no arrows for a field `F(x,y)`, and the plane has no camera: no panning, no zooming, no dragging the tip.
- The ⓘ panel of `obs-vector` relates **exactly two** vectors, or exactly two points. With three or more it lists each one on its own and says nothing about the pairs, because the block cannot know which pair you mean.
...
- `obs-vector` is two-dimensional and Cartesian: a line with three components (`(1,2,3)`) is not a pair, so it is typeset as free notation rather than drawn.
- The card layout of `obs-vector` grows up to four lines; from the fifth on, the cards share the panel height instead of making the block taller, so long blocks get small cards (each keeps its own scrollbar). The two-view toggle only splits the load when the block mixes declarations with an `AB`; four declared vectors still share one column.
...
and `G` in the **θᴅ / θʀ / θɢ** chip of `obs-trig` are the letters of Lora Italic, taken from the
